...
class ServerDetector {
    ...
    private static function detectBasePath($script_name) {
        ...
        $script_path = str_replace('\\', '/', $script_name);
        $script_dir = rtrim(str_replace('\\', '/', dirname($script_path)), '/');

        if ($script_dir === '' || $script_dir === '.' || $script_dir === '/') {
            return '';
        }

        $known_root_dirs = ['public', 'setup', 'scripts', 'sample', 'config', 'includes'];

        foreach ($known_root_dirs as $dir) {
            $pattern = '#/' . preg_quote($dir, '#') . '(/|$)#i';
            if (preg_match($pattern, $script_dir, $matches, PREG_OFFSET_CAPTURE)) {
                $position = $matches[0][1];
                $base = substr($script_dir, 0, $position);
                $base = rtrim($base, '/');
                return $base === '' ? '' : $base;
            }
        }

        // Fallback to script directory (handles installations within subdirectories)
