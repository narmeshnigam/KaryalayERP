...
function setup_document_vault()
{
		...
		if (table_exists_documents($conn, 'documents')) {
				closeConnection($conn);
				return ['success' => false, 'message' => 'documents table already exists.'];
		}
	...
	<div class="main-content">
		...
		<?php if ($result): ?>
			...
			<?php if ($auto_redirect): ?>
				<script>
					setTimeout(function() {
						window.location.href = '../public/documents/index.php';
					}, 2000);
				</script>
				<div class="alert alert-info" style="margin-top:16px;">
					Redirecting to Document Vault in 2 seconds...
				</div>
			<?php endif; ?>
		<?php endif; ?>
		...
		<div class="card" style="max-width:820px;margin:0 auto;">
			...
			<div style="margin:24px 0;">
				<?php if ($has_table): ?>
					<div class="alert alert-info" style="margin-bottom:16px;">
						The documents table already exists. You can start using the Document Vault.
					</div>
			...
			<div style="padding:16px;background:#f8f9fa;border-radius:8px;color:#6c757d;">
				After setup, upload employee, project, and administrative documents with visibility controls.
			</div>
		</div>
	</div>
</div>
