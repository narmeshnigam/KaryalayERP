<?php
/**
 * API: Mark Ready (In Progress → Ready to Deliver)
 */
...
try {
    ...
    // Verify delivery exists and is in progress
    ...
    if ($delivery['status'] !== 'In Progress') {
        throw new Exception('Only in-progress deliveries can be marked ready');
    }
    
    mysqli_begin_transaction($conn);
    
    // Update status
    $update_query = "UPDATE delivery_items SET status = 'Ready to Deliver' WHERE id = ?";
    ...
    $log_query = "INSERT INTO delivery_activity_log 
                  ...
                  VALUES (?, 'status_changed', 'Status changed to Ready to Deliver', ?)";
