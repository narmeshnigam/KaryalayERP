...
// initial form values from DB
$visitor_name = $visitor['visitor_name'];
$phone = $visitor['phone'];
$purpose = $visitor['purpose'];
$employee_id = (int) $visitor['employee_id'];
$check_in_time = date('Y-m-d\TH:i', strtotime($visitor['check_in_time']));
$existing_photo = $visitor['photo'];
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            if (empty($errors)) {
                ...
                if (!move_uploaded_file($file['tmp_name'], $destination_path)) {
                    ...
                    if (!empty($existing_photo) && $existing_photo !== $photo_path) {
                        $old_path = __DIR__ . '/../../' . ltrim($existing_photo, '/');
                        if (is_file($old_path)) {
                            @unlink($old_path);
                        }
                    }
