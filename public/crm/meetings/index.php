...
// Count total
$count_sql = "SELECT COUNT(*) as total FROM crm_meetings c $joins WHERE $where_clause";
...
$employees = [];
if ($has_assigned_to) {
  // Some installations may not have an `is_active` column on employees. Check available columns and adapt query.
...
$metrics_sql = "SELECT COUNT(*) as total,
                ...
                FROM crm_meetings WHERE deleted_at IS NULL";
...
$page_title = 'All Meetings - CRM - ' . APP_NAME;
    ...
    <div class="card">
      ...
      <?php else: ?>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
          <h3 style="margin:0;color:#003581;">📋 Meeting Directory (<?php echo count($meetings); ?>)</h3>
        </div>
            ...
            <tbody>
              <?php foreach ($meetings as $meeting): ?>
                ...
                <tr style="border-bottom:1px solid #e1e8ed;">
                  ...
                  <?php if ($has_assigned_to): ?>
                    <td style="padding:12px;">
                      <span style="background:#e3f2fd;color:#003581;padding:4px 10px;border-radius:12px;font-size:12px;font-weight:500;display:inline-block;">
                        <?php 
                          echo htmlspecialchars(crm_meeting_employee_label(
                  ...
                  <?php endif; ?>
                  <?php if ($has_outcome): ?>
                    <td style="padding:12px;font-size:13px;"><?php echo htmlspecialchars(crm_meeting_get($meeting, 'outcome', '—')); ?></td>
                  <?php endif; ?>
