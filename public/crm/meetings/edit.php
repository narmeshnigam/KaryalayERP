...
$has_updated_at = crm_meetings_has_column($conn, 'updated_at');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle file upload
    $attachment_filename = $meeting['attachment'] ?? '';
    ...
    if (empty($errors)) {
        ...
        $updates = ['title = ?', 'agenda = ?', 'meeting_date = ?'];
        $types = 'sss';
        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $errors[] = 'Failed to update meeting: ' . mysqli_error($conn);
            }
            mysqli_stmt_close($stmt);
        } else {
...
if (isset($meeting['meeting_date'])) {
    $dt = new DateTime($meeting['meeting_date']);
    ...
    <div class="page-header">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <a href="view.php?id=<?php echo $meeting_id; ?>" class="btn btn-secondary">Cancel</a>
          <a href="index.php" class="btn btn-accent">← All Meetings</a>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      <form method="POST" enctype="multipart/form-data">
          ...
          <div>
            <div class="form-group">
              ...
              <input type="text" name="title" class="form-control" required 
                     value="<?php echo htmlspecialchars($_POST['title'] ?? $meeting['title'] ?? ''); ?>"
            ...
            <div class="form-group">
              ...
              <input type="datetime-local" name="meeting_date" class="form-control" required
                     value="<?php echo htmlspecialchars($_POST['meeting_date'] ?? $meeting_datetime_local); ?>">
            </div>
              ...
              <select name="lead_id" class="form-control">
                ...
                <?php foreach ($leads as $lead): ?>
                  <option value="<?php echo $lead['id']; ?>" 
                          <?php echo (($_POST['lead_id'] ?? $meeting['lead_id'] ?? 0) == $lead['id']) ? 'selected' : ''; ?>>
              ...
              <select name="assigned_to" class="form-control">
                ...
                <?php foreach ($employees as $emp): ?>
                  <option value="<?php echo $emp['id']; ?>" 
                          <?php echo (($_POST['assigned_to'] ?? $meeting['assigned_to'] ?? 0) == $emp['id']) ? 'selected' : ''; ?>>
            ...
            <div class="form-group">
              ...
              <input type="text" name="location" class="form-control"
                     value="<?php echo htmlspecialchars($_POST['location'] ?? $meeting['location'] ?? ''); ?>"
          ...
          <div>
            <div class="form-group">
              ...
              <textarea name="agenda" class="form-control" rows="5" required
                        placeholder="Meeting discussion points and objectives..."><?php echo htmlspecialchars($_POST['agenda'] ?? $meeting['agenda'] ?? ''); ?></textarea>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea name="outcome" class="form-control" rows="4"
                        placeholder="Meeting summary and decisions..."><?php echo htmlspecialchars($_POST['outcome'] ?? $meeting['outcome'] ?? ''); ?></textarea>
            </div>
            <?php endif; ?>
            ...
            <div class="form-group">
              ...
              <input type="date" name="follow_up_date" class="form-control"
                     value="<?php echo htmlspecialchars($_POST['follow_up_date'] ?? $meeting['follow_up_date'] ?? ''); ?>"
              ...
              <select name="follow_up_type" class="form-control">
                ...
                <?php foreach (crm_meeting_follow_up_types() as $type): ?>
                  <option value="<?php echo $type; ?>" 
                          <?php echo (($_POST['follow_up_type'] ?? $meeting['follow_up_type'] ?? '') === $type) ? 'selected' : ''; ?>>
            ...
            <div class="form-group">
              ...
              <?php if (!empty($meeting['attachment'])): ?>
                <div style="margin-bottom:8px;padding:8px;background:#f8f9fa;border-radius:6px;font-size:13px;">
                  Current: <a href="../../../uploads/crm_attachments/<?php echo htmlspecialchars($meeting['attachment']); ?>" target="_blank" style="color:#003581;">
                    <?php echo htmlspecialchars($meeting['attachment']); ?>
                  </a>
                </div>
              <?php endif; ?>
        ...
        <div style="margin-top:24px;padding-top:20px;border-top:1px solid #dee2e6;display:flex;gap:12px;justify-content:flex-end;">
          <a href="view.php?id=<?php echo $meeting_id; ?>" class="btn btn-secondary">Cancel</a>
          <button type="submit" class="btn">💾 Update Meeting</button>
        </div>
      </form>
    </div>
  </div>
