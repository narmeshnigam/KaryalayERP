...
$change_logs = [];
if (!$not_found) {
    $inventory_logs_raw = get_item_inventory_log($conn, $item_id);
    $files_raw = get_item_files($conn, $item_id);
    $change_logs_raw = get_item_change_log($conn, $item_id);

    $inventory_logs = is_array($inventory_logs_raw) ? $inventory_logs_raw : [];
    $files = is_array($files_raw) ? $files_raw : [];
    $change_logs = is_array($change_logs_raw) ? $change_logs_raw : [];
}
...
if (!$not_found) {
    ...
    $itemExpiry = $item['expiry_date'] ?? null;
    $basePrice = $item['base_price'] ?? null;
    $taxPercent = $item['tax_percent'] ?? null;
    $defaultDiscount = $item['default_discount'] ?? null;
    ...
    $primaryPath = trim((string)($item['primary_image'] ?? ''));
    if ($primaryPath !== '') {
        $primaryFs = __DIR__ . '/../../' . ltrim($primaryPath, '/');
        $primaryUrl = APP_URL . '/' . ltrim($primaryPath, '/');
        $imageExists = file_exists($primaryFs);
    }

    $brochurePath = trim((string)($item['brochure_pdf'] ?? ''));
    if ($brochurePath !== '') {
        $brochureFs = __DIR__ . '/../../' . ltrim($brochurePath, '/');
        $brochureUrl = APP_URL . '/' . ltrim($brochurePath, '/');
        $brochureExists = file_exists($brochureFs);
    }
}

// All logged-in users can edit and delete in catalog
$can_edit = true;
$can_delete = true;
...
$page_title = ($not_found ? 'Item Not Found' : safeValue($itemName)) . ' - Catalog - ' . APP_NAME;
            ...
            <div class="card" style="display: flex; gap: 20px; align-items: center;">
                ...
                <?php if ($imageExists): ?>
                    <img src="<?php echo htmlspecialchars($primaryUrl); ?>" 
                         alt="<?php echo safeValue($itemName); ?>" 
                ...
                <div style="flex: 1;">
                    <div style="font-size: 24px; color: #003581; font-weight: 700; margin-bottom: 8px;">
                        <?php echo safeValue($itemName); ?>
                        <span style="font-size: 14px; color: #6c757d; font-weight: 500; margin-left: 8px; font-family: monospace;">
                            (SKU: <?php echo safeValue($itemSku); ?>)
                        </span>
                    </div>
                    <div style="color: #6c757d; font-size: 14px; margin-bottom: 10px;">
                        Category: <?php echo safeValue($itemCategory, 'Uncategorized'); ?>
                    </div>
                    <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                        <span class="badge-custom <?php echo $isProduct ? 'badge-product' : 'badge-service'; ?>">
                            <?php echo $isProduct ? '📦 Product' : '🛠️ Service'; ?>
                        </span>
                        <span class="badge-custom <?php echo $itemStatus === 'Active' ? 'badge-active' : 'badge-inactive'; ?>">
                            <?php echo safeValue($itemStatus); ?>
                        </span>
                        <?php if ($isProduct && $isLowStock): ?>
                ...
                <div style="text-align: right;">
                    ...
                    <div style="font-size: 28px; font-weight: 700; color: #003581;">
                        <?php echo formatCurrency($basePrice); ?>
                    </div>
                ...
                <div class="info-card">
                    ...
                    <div class="info-grid">
                        ...
                        <div>
                            ...
                            <span><?php echo formatNumber($taxPercent, 2); ?>%</span>
                        </div>
                        <div>
                            <strong>Default Discount</strong>
                            <span><?php echo formatCurrency($defaultDiscount); ?></span>
                        </div>
                        <div>
                            <strong>Final Price (with tax)</strong>
                            <span style="color: #28a745; font-weight: 600;">
                                <?php 
                                    $basePriceValue = (float)($basePrice ?? 0);
                                    $taxPercentValue = (float)($taxPercent ?? 0);
                                    $discountValue = (float)($defaultDiscount ?? 0);
                                    $netPrice = max(0, $basePriceValue - $discountValue);
                                    $finalPrice = $netPrice * (1 + $taxPercentValue / 100);
                ...
                <div class="info-card">
                    ...
                    <div class="info-grid">
                        <div>
                            ...
                            <span style="font-size: 24px; font-weight: 700; color: <?php echo $isLowStock ? '#f57c00' : '#28a745'; ?>;">
                                <?php echo formatNumber($currentStock); ?>
                            </span>
                        </div>
                        <div>
                            <strong>Low Stock Threshold</strong>
                            <span><?php echo formatNumber($lowStockThreshold, 0, 'Not set'); ?></span>
                        </div>
                        <div style="grid-column: 1 / -1;">
                            <strong>Stock Value</strong>
                            <span style="font-size: 18px; font-weight: 600; color: #003581;">
                                <?php 
                                    $stockValue = (float)($currentStock ?? 0) * (float)($basePrice ?? 0);
                ...
                <div class="info-card">
                    ...
                    <div class="info-grid">
                        <div>
                            ...
                            <span><?php echo safeValue($itemType); ?></span>
                        </div>
                        <div>
                            <strong>Category</strong>
                            <span><?php echo safeValue($itemCategory, 'Uncategorized'); ?></span>
                        </div>
                        <div>
                            <strong>Status</strong>
                            <span><?php echo safeValue($itemStatus); ?></span>
                        </div>
                        <div>
                            <strong>Expiry Date</strong>
                            <span><?php echo safeDate($itemExpiry); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="info-card">
                    ...
                    <div class="info-grid">
                        <div>
                            ...
                            <span><?php echo safeValue($createdByName, 'System'); ?></span>
                        </div>
                        <div>
                            <strong>Created At</strong>
                            <span><?php echo safeDateTime($createdAt); ?></span>
                        </div>
                        <div style="grid-column: 1 / -1;">
                            <strong>Last Updated</strong>
                            <span><?php echo safeDateTime($updatedAt); ?></span>
                        </div>
                    </div>
                </div>
            ...
            <div class="card" style="margin-top: 30px;">
                <div class="tab-navigation">
                          <a href="?id=<?php echo $item_id; ?>&tab=inventory" 
                              class="tab-link <?php echo $active_tab === 'inventory' ? 'tab-active' : ''; ?>">
                        📦 Inventory Log (<?php echo count($inventory_logs); ?>)
                    </a>
                          <a href="?id=<?php echo $item_id; ?>&tab=files" 
                              class="tab-link <?php echo $active_tab === 'files' ? 'tab-active' : ''; ?>">
                        📎 Files (<?php echo count($files); ?>)
                    </a>
                          <a href="?id=<?php echo $item_id; ?>&tab=history" 
                              class="tab-link <?php echo $active_tab === 'history' ? 'tab-active' : ''; ?>">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($inventory_logs as $log): ?>
                                        <?php
                                            ...
                                            $style = $inventoryActionStyles[$logAction] ?? 'background: #e2e3e5; color: #383d41;';
                                            $quantityDelta = (float)($log['quantity_delta'] ?? 0);
                                        ?>
                                        <tr style="border-bottom: 1px solid #dee2e6;">
                                            <td style="padding: 12px; font-size: 13px;">
                                                <?php echo safeDateTime($log['created_at'] ?? null); ?>
                                            </td>
                                            <td style="padding: 12px;">
                                                <span style="<?php echo $style; ?> padding: 4px 10px; border-radius: 12px; font-size: 12px; font-weight: 600; display: inline-block;">
                                                    <?php echo safeValue($logAction); ?>
                                                </span>
                                            </td>
                                            <td style="padding: 12px; text-align: right; font-weight: 600; font-size: 15px; color: <?php echo $quantityDelta > 0 ? '#28a745' : '#dc3545'; ?>;">
                                                <?php echo $quantityDelta > 0 ? '+' : ''; ?><?php echo formatNumber($quantityDelta); ?>
                                            </td>
                                            <td style="padding: 12px; text-align: center; font-weight: 600;">
                                                <?php echo formatNumber($log['qty_before'] ?? null); ?>
                                            </td>
                                            <td style="padding: 12px; text-align: center; font-weight: 600;">
                                                <?php echo formatNumber($log['qty_after'] ?? null); ?>
                                            </td>
                                            <td style="padding: 12px; font-size: 13px; color: #6c757d;">
                                                <?php echo safeValue($log['reason'] ?? ''); ?>
                                            </td>
                                            <td style="padding: 12px; font-size: 13px;">
                                                <?php echo safeValue($log['created_by_name'] ?? null, 'System'); ?>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                                ...
                                <tbody>
                                    <?php foreach ($files as $file): ?>
                                        ...
                                        <tr style="border-bottom: 1px solid #dee2e6;">
                                            ...
                                            <td style="padding: 12px; font-size: 13px;">
                                                <?php echo safeValue($file['uploaded_by_name'] ?? null, 'Unknown'); ?>
                                            </td>
                                            <td style="padding: 12px; font-size: 13px; color: #6c757d;">
                                                <?php echo safeDateTime($file['uploaded_at'] ?? null); ?>
                                            </td>
                                ...
                                <tbody>
                                    <?php foreach ($change_logs as $log): ?>
                                        <tr style="border-bottom: 1px solid #dee2e6;">
                                            ...
                                            <td style="padding: 12px; font-size: 13px;">
                                                <?php echo safeValue($log['changed_by_name'] ?? null, 'System'); ?>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
