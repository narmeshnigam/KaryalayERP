<?php
/**
 * API: Approve Deliverable Internally
 */
...
try {
    ...
    if (!$current || $current['status'] !== 'Submitted') {
        throw new Exception("Deliverable cannot be approved in current status");
    }

    // Update deliverable status
    $update_query = "UPDATE deliverables SET status = 'Internal Approved', updated_at = NOW() WHERE id = ?";
    ...
    $version_query = "UPDATE deliverable_versions 
        SET approval_internal = 1, approved_by_internal = ?, approval_date_internal = NOW() 
        WHERE deliverable_id = ? AND version_no = ?";
    $version_stmt = mysqli_prepare($conn, $version_query);
    mysqli_stmt_bind_param($version_stmt, 'iii', $user_id, $deliverable_id, $current['current_version']);
    ...
    $activity_query = "INSERT INTO deliverable_activity_log 
        ...
        VALUES (?, ?, 'Approve Internal', 'Deliverable approved internally', NOW())";
    ...
    $_SESSION['flash_message'] = "Deliverable approved internally!";
