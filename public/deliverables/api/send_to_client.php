<?php
/**
 * API: Send Deliverable to Client for Review
 */
...
try {
    ...
    if (!$current || $current['status'] !== 'Internal Approved') {
        throw new Exception("Deliverable must be internally approved first");
    }

    // Update status
    $update_query = "UPDATE deliverables SET status = 'Client Review', updated_at = NOW() WHERE id = ?";
    ...
    $activity_query = "INSERT INTO deliverable_activity_log 
        ...
        VALUES (?, ?, 'Update', 'Deliverable sent to client for review', NOW())";
    ...
    $_SESSION['flash_message'] = "Deliverable sent to client for review!";
