                ...
                <table id="permissionsMatrix" style="width:100%;border-collapse:collapse;font-size:12px;">
                    ...
                    <tbody>
                        ...
                        ?>
                            ...
                            <tr class="module-row" data-module="<?php echo $module_id; ?>" style="background:#e3f2fd;">
...
// Helper to toggle a checkbox and trigger its change handler
...
// Row-level select/clear for a specific role within a row
document.querySelectorAll('.row-select').forEach(btn => {
    btn.addEventListener('click', function() {
        const action = this.getAttribute('data-action');
        const roleId = this.getAttribute('data-role');
        const row = this.closest('tr.table-row');
        if (!row) return;
        const value = action === 'select';
        row.querySelectorAll(`input.perm-checkbox[data-role="${roleId}"]`).forEach(cb => toggleCheckbox(cb, value));
    });
});

// Module-level select/clear for a specific role across all tables in that module
document.querySelectorAll('.module-select').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        const value = action === 'select';
        document.querySelectorAll(`tr.table-row[data-module="${moduleId}"] input.perm-checkbox[data-role="${roleId}"]`)
            .forEach(cb => toggleCheckbox(cb, value));
    });
});
</script>
